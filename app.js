...
// --- SECURITY HELPERS ---
// Escapes regex special characters so raw user search input can't break
// the highlight regex or be used to craft an unexpected pattern.
function escapeRegex(str) {
    return String(str).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

// Telegram uses legacy Markdown for the pre-filled order message (see the
// `*NEW ORDER*` bolding in submitFinalOrder). Any literal *, _, `, or [ typed
// by the customer into phone/address/note fields would otherwise break the
// formatting or let them inject fake bold/links into the message you receive.
function escapeMarkdown(str) {
    return String(str ?? '').replace(/([_*`\[])/g, '\\$1');
}

// Recomputes an order line from the product catalog itself (the source of
// truth) rather than trusting whatever is sitting in the cart object. A
// tampered localStorage entry (e.g. someone editing cartPrice in devtools)
// can no longer change what price actually reaches your order message.
function getCanonicalLineItem(productId, variantName, quantity) {
    const p = products.find(item => item.id === productId);
    if (!p) return null;

    let variant = (p.variants || []).find(v => v.name === variantName);
    if (!variant) variant = { name: 'Standard', price: p.price };

    const qty = Number.isFinite(quantity) && quantity > 0 ? quantity : 1;
    return { id: p.id, name: p.name, image: p.image, variant, quantity: qty, price: Number(variant.price) };
}
...
const app = {
    ...
    init() {
        ...
        try { 
            ...
            if (savedCart) {
                ...
                // Rebuild each entry from the product catalog so a stale or
                // hand-edited localStorage value can never carry a wrong price.
                this.cart = (Array.isArray(parsed) ? parsed : [])
                    .map(item => {
                        const canonical = getCanonicalLineItem(item?.id, item?.variant?.name, item?.quantity);
                        return canonical ? { ...canonical, cartPrice: canonical.price } : null;
                    })
                    .filter(Boolean);
            }
        } catch(e) { 
    ...
    highlightText(text) { 
        ...
        const regex = new RegExp(`(${escapeRegex(this.searchQuery)})`, 'gi'); 
    ...
    submitFinalOrder() {
        ...
        // Compile Order Data — rebuilt from the product catalog (source of
        // truth) so a tampered currentVariant/cart value in memory or
        // localStorage can never change what price you actually see.
        let items = []; 
        let total = 0; 
        
        if (this.pendingOrderProductId) { 
            const variantName = this.currentVariant?.name;
            const line = getCanonicalLineItem(this.pendingOrderProductId, variantName, 1);
            if (line) { items.push(line); total = line.price; }
        } else { 
            items = this.cart
                .map(i => getCanonicalLineItem(i.id, i.variant?.name, i.quantity))
                .filter(Boolean);
            items.forEach(i => total += i.price * i.quantity);
        }
        
        let msg = `🛒 *NEW ORDER*\nMethod: ${escapeMarkdown(d)}\n\n`;
        if (isStandard) {
            msg += `🏢 Company: ${escapeMarkdown(this.selectedCompany)}\n`;
            msg += `📍 Province: ${escapeMarkdown(document.getElementById('modal-province').value)}\n`;
            msg += `🏠 Address: ${escapeMarkdown(document.getElementById('modal-address').value) || 'N/A'}\n`;
            msg += `📞 Phone: ${escapeMarkdown(document.getElementById('modal-phone').value)}\n\n`;
        }
        if (isGrab) {
            msg += `📍 Location: http://googleusercontent.com/maps.google.com/maps?q=${this.mapLat},${this.mapLng}\n`;
            msg += `📞 Phone: ${escapeMarkdown(document.getElementById('grab-phone').value)}\n\n`;
        }
        
        const note = document.getElementById('modal-note').value.trim();
        msg += `📝 Note: ${note ? escapeMarkdown(note) : 'None'}\n\n`;
        
        msg += `📦 *Items:*\n`;
        msg += items.map((i, idx) => `${idx+1}. ${escapeMarkdown(i.name)} (${escapeMarkdown(i.variant?.name || 'Standard')}) - ${i.quantity}x @ $${i.price.toFixed(2)}`).join('\n');
